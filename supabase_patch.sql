...
DROP POLICY IF EXISTS "Tenants can view invoices by email" ON invoices;
...
DROP POLICY IF EXISTS "Landlords can manage their payment methods" ON payment_methods;
CREATE POLICY "Landlords can manage their payment methods" ON payment_methods
  FOR ALL USING (landlord_id = auth.uid())
  WITH CHECK (landlord_id = auth.uid());

DROP POLICY IF EXISTS "Tenants can view payment methods for their leases" ON payment_methods;
...
DROP POLICY IF EXISTS "Landlords can manage payment submissions" ON payment_submissions;
CREATE POLICY "Landlords can manage payment submissions" ON payment_submissions
  FOR ALL USING (landlord_id = auth.uid())
  WITH CHECK (landlord_id = auth.uid());

DROP POLICY IF EXISTS "Tenants can create payment submissions" ON payment_submissions;
CREATE POLICY "Tenants can create payment submissions" ON payment_submissions
  FOR INSERT WITH CHECK (tenant_id = auth.uid());

DROP POLICY IF EXISTS "Tenants can view their payment submissions" ON payment_submissions;
